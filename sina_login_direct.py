...
def login(username, password):
    ...
    headers["Referer"] = "https://weibo.com/u/2786328714/home"
    ...
    user_name = re.findall(ra, weibo_page.content.decode("utf-8", 'ignore').replace('\\',''), re.S)[0]
    print(user_name)
    url_r = 'https:'+user_name
    weibo_pag = session.get(url_r, headers=headers)
    print(weibo_pag.content.decode("utf-8", 'ignore'))
    rb = r'<a.+?action-data=\".*mid=(.+?)&like.*\">'
    mid = re.findall(rb, weibo_pag.content.decode("utf-8", 'ignore').replace('\\',''), re.S)[0]
    print(mid)
    pao['mid'] = mid
    rs = r'<div.+?diss-data=\".*rid=(.+?)\".*>'
    rid = re.findall(rs, weibo_pag.content.decode("utf-8", 'ignore').replace('\\', ''), re.S)[0]
    print(rid)
    pao['rid'] = rid
    h2 = 'https://weibo.com/aj/v6/like/add?ajwvr=6&__rnd='
    h3 = h2+str(int(time.time() * 1000))
    print(h3)
    hi = requests.post(h3,data=pao,headers=headers)
    hi = session.post(h3,data=pao,headers=headers)
    print(hi.text)
